...
UINT_PTR Dumper::GetMethodOffset(const char* assembly_name, const char* _namespace, const char* class_name, const char* method_name, int args)
{
	...
	if (_namespace != "")
	{
		printf("[Dumped Method] %s.%s.%s.%s : [0x%X]", current_assembly_name, method._namespace, method.class_name, method.method_name, method.Pointer);
	}
	else
	{
		printf("[Dumped Method] %s.%s.%s : [0x%X]", current_assembly_name, method.class_name, method.method_name, method.Pointer);
	}
	return method.Pointer;
}
...
UINT_PTR Dumper::GetFieldOffet(const char* assembly_name, const char* _namespace, const char* class_name, const char* field_name)
{
	...
	if (_namespace != "")
	{
		printf("[Dumped Field] %s.%s.%s.%s : [0x%X]", current_assembly_name, field._namespace, field.class_name, field.field_name, field.Pointer);
	}
	else
	{
		printf("[Dumped Field] %s.%s.%s : [0x%X]", current_assembly_name, field.class_name, field.field_name, field.Pointer);
	}
	return field.Pointer;
}